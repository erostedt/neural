...
network_t network_alloc(size_t batch_size, size_t* node_counts, size_t size)
{
    assert(size > 1);
    network_t network;
    network.layers = malloc((size-1) * sizeof(layer_t));

    for (size_t layer_index = 0; layer_index < size-1; ++layer_index)
    {
        layer_t layer = layer_alloc(batch_size, node_counts[layer_index], node_counts[layer_index+1]);
        layer_randomize(&layer);
        network.layers[layer_index] = layer;
    }
    network.layer_count = size-1;
    network.loss = loss_alloc(batch_size, node_counts[size-1]);
    return network;
}
