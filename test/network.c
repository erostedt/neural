...
{
    layer_type_t layers[] = {
        LAYER_SIGMOID(1), LAYER_RELU(2), LAYER_TANH(3), LAYER_LINEAR(4), LAYER_SOFTMAX(5),
    };
    ...
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[0].state.m_weights, network.layers[0].state.v_weights, (matrix_t){2, 1}));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[1].state.m_weights, network.layers[1].state.v_weights, (matrix_t){1, 2}));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[2].state.m_weights, network.layers[2].state.v_weights, (matrix_t){2, 3}));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[3].state.m_weights, network.layers[3].state.v_weights, (matrix_t){3, 4}));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[4].state.m_weights, network.layers[4].state.v_weights, (matrix_t){4, 5}));
    ...
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[0].outputs, network.layers[0].d_outputs, network.layers[0].activations));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[1].outputs, network.layers[1].d_outputs, network.layers[1].activations));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[2].outputs, network.layers[2].d_outputs, network.layers[2].activations));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[3].outputs, network.layers[3].d_outputs, network.layers[3].activations));
    ASSERT_TRUE(
        matrix_same_shapes(network.layers[4].outputs, network.layers[4].d_outputs, network.layers[4].activations));
    ...
    ASSERT_TRUE(vector_same_shapes(network.layers[0].biases, network.layers[0].state.m_biases,
                                   network.layers[0].state.v_biases));
    ASSERT_TRUE(vector_same_shapes(network.layers[1].biases, network.layers[1].state.m_biases,
                                   network.layers[1].state.v_biases));
    ASSERT_TRUE(vector_same_shapes(network.layers[2].biases, network.layers[2].state.m_biases,
                                   network.layers[2].state.v_biases));
    ASSERT_TRUE(vector_same_shapes(network.layers[3].biases, network.layers[3].state.m_biases,
                                   network.layers[3].state.v_biases));
    ASSERT_TRUE(vector_same_shapes(network.layers[4].biases, network.layers[4].state.m_biases,
                                   network.layers[4].state.v_biases));
}
